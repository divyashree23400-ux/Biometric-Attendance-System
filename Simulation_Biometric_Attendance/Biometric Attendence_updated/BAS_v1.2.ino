/*
  ==========================================================
  TITUS SOLUTIONS - Biometric Attendance System
  ESP32-S3 | ILI9341 TFT | DS1307 RTC
  MFRC522 RFID | Adafruit/R307 Fingerprint Sensor
  Author: Sai Prabhat CA

  SIMULATION VERSION (Wokwi-friendly):
  - No SD card. Student roster is hardcoded below in
...
void setup() {
  ...
  // Header for the attendance log, printed once at boot
  Serial.println("RollNo,Name,Department,Method,Date,Time");
...
void populateStudents() {
  ...
  students.push_back({ "24BTREC043", "Sai Prabhat CA", "12345678", 1, "ECE" });
  students.push_back({ "24BTREC044", "Vinay",     "11223344", 2, "CSE" });
  students.push_back({ "24BTREC045", "Sam",    "22334455", 3, "EEE" });
  students.push_back({ "24BTREC046", "Chai",     "33445566", 4, "MECH" });

  // Add more students the same way:
  // students.push_back({ "ROLL_NO", "Name", "RFID_UID", FINGERPRINT_ID, "DEPT" });
}
...
void markAttendance(Student* stu, const char* method) {
  if (stu == nullptr) {
    denyFeedback();
    showResultScreen(nullptr, method);
    return;
  }
  ...
  // Logged to Serial instead of SD card CSV — same column format
  Serial.printf("%s,%s,%s,%s,%s,%s\n",
    stu->rollNo.c_str(), stu->name.c_str(), stu->department.c_str(),
    method, dateStr, timeStr);

  grantFeedback();
  showResultScreen(stu, method);
}
...
void animateTick(int x, int y) {
  ...
  animateLine(x, y, x + 15, y + 15, ILI9341_GREEN, 2);
  // Draw up-right stroke
  animateLine(x + 15, y + 15, x + 40, y - 20, ILI9341_GREEN, 2);
}
...
void showResultScreen(Student* stu, const char* method) {
  ...
  if (stu == nullptr) {
    ...
    // Draw animated tick at the bottom right corner
    animateTick(260, 190); 
  }
